...
const bracelets = [
  ...
  {
    id: 8,
    name: "Stackable Ring Bracelet Set",
    price: 799,
    originalPrice: 999,
    image: "/placeholder.svg?height=300&width=300",
    rating: 4.4,
...
const sizeOptions = [
  { value: "small", label: "Small (6.5-7 inch)" },
  { value: "medium", label: "Medium (7-7.5 inch)" },
  { value: "large", label: "Large (7.5-8 inch)" },
...
export default function BraceletsPage() {
  ...
  const filteredBracelets = bracelets.filter((bracelet) => {
    const matchesSearch = bracelet.name.toLowerCase().includes(searchQuery.toLowerCase())
    const matchesPrice = bracelet.price >= priceRange[0] && bracelet.price <= priceRange[1]
    const matchesGemstone = selectedGemstones.length === 0 || selectedGemstones.includes(bracelet.gemstone)
    const matchesMetal = selectedMetals.length === 0 || selectedMetals.includes(bracelet.metal)
    const matchesStyle = selectedStyles.length === 0 || selectedStyles.includes(bracelet.style)
    const matchesSize =
      selectedSizes.length === 0 ||
      selectedSizes.includes(bracelet.size) ||
      selectedSizes.some((size) => {
        if (bracelet.size === "adjustable" || bracelet.size === "one-size") return selectedSizes.includes(bracelet.size)
        const braceletInch = Number.parseFloat(bracelet.size)
        switch (size) {
          case "small":
            return braceletInch >= 6.5 && braceletInch <= 7
          case "medium":
            return braceletInch >= 7 && braceletInch <= 7.5
          case "large":
            return braceletInch >= 7.5 && braceletInch <= 8
          default:
            return true
        }
      })
    const matchesStock = !showOnlyInStock || bracelet.inStock
    const matchesCustomizable = !showOnlyCustomizable || bracelet.customizable
    ...
    <div className="min-h-screen bg-stone-50">
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex space-x-6">
              ...
              <Link href="/bracelets" className="text-emerald-600 font-medium border-b-2 border-emerald-600">
                Bracelets
              </Link>
            ...
            <div className="flex items-center space-x-4">
              <div className="hidden md:flex items-center space-x-2">
                ...
                <Input placeholder="Search bracelets..." className="w-64" />
              </div>
      ...
      <section className="bg-gradient-to-r from-amber-600 to-orange-700 text-white py-16">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-5xl font-serif font-bold mb-4">Beautiful Bracelets</h1>
          <p className="text-xl opacity-90 mb-8">
            Adorn your wrists with elegance - from delicate chains to statement cuffs
          </p>
          <div className="flex justify-center space-x-4">
            <Button variant="outline" className="text-amber-700 border-white bg-white hover:bg-stone-100" asChild>
              <Link href="/customize">Design Your Bracelet</Link>
            </Button>
            <Button variant="outline" className="border-white text-white hover:bg-white hover:text-amber-700" asChild>
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1">
            <Card className="p-6 space-y-6 sticky top-4">
              ...
              <div className="space-y-4">
                ...
                <Slider value={priceRange} onValueChange={setPriceRange} max={6000} step={100} className="w-full" />
          ...
          <div className="lg:col-span-3">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-4">
                <span className="text-stone-600">{sortedBracelets.length} bracelets found</span>
              </div>
                    ...
                    {viewMode === "grid" ? (
                      <>
                        <div className="relative overflow-hidden rounded-t-lg">
                          <Link href={`/product/${bracelet.id}`}>
                            <Image
                              src={bracelet.image || "/placeholder.svg"}
                              alt={bracelet.name}
                              width={300}
                              height={300}
                          ...
                          <div className="absolute top-3 left-3 flex flex-col space-y-1">
                            {bracelet.originalPrice && (
                              <Badge className="bg-red-500">Save ${bracelet.originalPrice - bracelet.price}</Badge>
                            )}
                            {bracelet.bestseller && <Badge className="bg-emerald-600">Bestseller</Badge>}
                            {bracelet.newArrival && <Badge className="bg-blue-600">New</Badge>}
                            {!bracelet.inStock && <Badge className="bg-gray-500">Out of Stock</Badge>}
                          </div>
                        ...
                        <div className="p-6 space-y-3">
                            ...
                            <div className="flex items-center">
                              {[...Array(5)].map((_, i) => (
                                <Star
                                  key={i}
                                  className={`h-4 w-4 ${
                                    i < Math.floor(bracelet.rating) ? "text-yellow-400 fill-current" : "text-stone-300"
                                  }`}
                                />
                              ))}
                            </div>
                            <span className="text-sm text-stone-600">({bracelet.reviews})</span>
                          </div>

                          <div className="flex items-center space-x-2">
                            <span className="text-2xl font-bold text-stone-800">${bracelet.price}</span>
                            {bracelet.originalPrice && (
                              <span className="text-lg text-stone-400 line-through">${bracelet.originalPrice}</span>
                            )}
                          </div>

                          <p className="text-sm text-stone-600">{bracelet.size}</p>
                          ...
                          <div className="flex space-x-2 pt-2">
                            ...
                            <Button variant="outline" className="flex-1" disabled={!bracelet.inStock}>
                              {bracelet.inStock ? "Add to Cart" : "Out of Stock"}
                            </Button>
                          </div>
                        </div>
                      </>
                      ...
                      <div className="flex items-center space-x-6 p-6">
                        ...
                        <div className="flex flex-col space-y-2">
                          ...
                          <Button variant="outline" disabled={!bracelet.inStock}>
                            {bracelet.inStock ? "Add to Cart" : "Out of Stock"}
                          </Button>
                        </div>
                      </div>
            ...
            {sortedBracelets.length === 0 && (
              <div className="text-center py-12">
                <p className="text-lg text-stone-600 mb-4">No bracelets found matching your criteria</p>
